= AsciiDoc for iTC Syntax Reference
:showtitle:
:toc:
:sectnums:
:imagesdir: images
...
== Overview
This document is not really for reading, but is to show various syntax/styling for AsciiDoc that would be expected to be used by an iTC in creating/maintaining their documentation.
...
These guides have the full syntax supporting by AsciiDoc, while this one is focused on the syntax that is used in the templates provided here and expected to be used in these documents.
...
=== Definitions
Another type of list in AsciiDoc is a "definition".
...
You can add text in the [], and the numbers are the size of the image. Note that the image will be presenting with the same ratio when displayed.

To place an image on its own with a lable, use the below example.
